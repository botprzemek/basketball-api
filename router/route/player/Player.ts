...
import {Player as PlayerBuilder} from 'model/builder/Player'
...
router.get('/', async (req: Request, res: Response): Promise<Response> => {
    ...
    console.log(`${new Date().toLocaleTimeString('pl-PL')} [request] GET ${decodeURI(req.baseUrl + req.path)} - requested players (${((performance.now() - res.locals.start) / 1000).toFixed(2)}s)`)
})
