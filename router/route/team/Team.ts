...
router.get('/', async (req: Request, res: Response): Promise<Response> => {
    ...
    res.send(data)
    console.log(`${new Date().toLocaleTimeString('pl-PL')} [request] GET ${decodeURI(req.baseUrl + req.path)} - requested teams (${((performance.now() - res.locals.start) / 1000).toFixed(2)}s)`)
})
...
router.get('/:team', async (req: Request, res: Response): Promise<Response> => {
    ...
    res.send(data)
    console.log(`${new Date().toLocaleTimeString('pl-PL')} [request] GET ${decodeURI(req.baseUrl + req.path)} - requested team (${((performance.now() - res.locals.start) / 1000).toFixed(2)}s)`)
})
